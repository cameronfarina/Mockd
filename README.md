...
`draft:ui` starts the local live draft room at `http://localhost:4317` by default. The board is searchable and uses `+` actions to stage a player for a selected team and sale price. The side panel shows a selectable owner roster with QB/RB/RB/WR/WR/TE/FLEX/K/DST/bench slots, owner budgets, and recent sales. The board shows expected price, live room-adjusted price, Cam-specific personal value, recommended max bid, team, and bye week. Players outside the priced auction pool can still resolve from the 2026 ESPN projection dataset and are marked as projection fallback targets. The 2026 bye-week metadata lives in `config/nflTeams.ts`.
