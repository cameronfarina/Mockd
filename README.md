...
`draft:ui` starts the local draft room at `http://localhost:4317` by default. The left rail has three distinct draft actions: **Start real draft** for draft-night sale logging, **Start mock draft** for an interactive practice room where Cam controls Cam and AI owners control everyone else, and **Run mocks** for read-only AI-only batch simulations. Start mock opens a practice room automatically when the live room is locked, while Start real draft returns to the live room. The board is searchable, filterable by position, selected-owner needs, NFL team, and bye week, and sortable by the main draft-day economics columns. It uses `+` actions to stage a player for a selected team and sale price, and the star next to each draftable player saves them to Cam's local shortlist. Pre-draft keeper rows are shown greyed out with the keeper owner/cost, but they are disabled and drop out of the board after real draft sales begin. The board defaults to expected draft price order, with season points available as the next clean sort for projection-first review. It shows ESPN Week 1 and full-season projection points, expected price, live room-adjusted price, owner-specific personal value, value gap, recommended max bid, team, bye week, tier-drop tags, and position-market pressure. The board stays room-wide: when the selected owner cannot legally roster a position, those targets remain visible for search and other-owner context, but their owner-specific `Our` and `Max` values drop to `$0` and they stay out of Cam's shortlist/path recommendations. The strategy selector changes Cam's personal value, tags, shortlist, and recommended bids in real time without changing the league market anchor. `Our` remains Cam's strategy value, while `Max` respects the selected strategy's path discipline. For true-three-RB, the first RB can flex up, but the next RB maxes compress as Cam spends into the core RB budget envelope. The header also accepts compact sale commands such as `jakub kittle 28`. The right panel defaults to the selected owner's lineup and budget context, with tabs for My shortlist and Draft path so the dense mock, readiness, blockers, and audit detail can take over the panel only when needed. Players outside the priced auction pool can still resolve from the 2026 ESPN projection dataset and are marked as projection fallback targets. The 2026 bye-week metadata lives in `config/nflTeams.ts`.

The same local SPA also serves `/my-expert`, an advice-only season assistant. It analyzes Cam's active Mockd roster and available board targets, then returns weekly lineup/flex advice, add/drop, bye coverage, injury watch, and trade-target cards through `/api/my-expert`. Lineup cards rank legal starters and FLEX candidates with adjusted projection, evidence, and risk fields. Every recommendation is marked read-only: Mockd can recommend what to review, but it does not submit waiver claims, trades, lineup changes, or platform actions. `/api/sync/providers` exposes the read-only provider readiness contract for Mockd draft, Sleeper, Yahoo, and ESPN. Sleeper is available as the no-OAuth first sync path, Yahoo is gated behind `MOCKD_YAHOO_CLIENT_ID` and `MOCKD_YAHOO_CLIENT_SECRET` plus `/api/sync/oauth/yahoo/start`, and ESPN is gated behind local-only credentials (`MOCKD_ESPN_LEAGUE_ID`, `MOCKD_ESPN_SWID`, `MOCKD_ESPN_S2`) until a provider-approved hosted access path exists.

The same local SPA also serves `/player-news`, a Rotoworld-style player update feed. By default it combines the checked-in sourced evidence file with the no-key RotoWire NFL RSS feed when that feed is reachable, then displays news cards with player, category, source, draft action, current availability, live price, and recommended max bid context. The source selector can narrow the feed back to local evidence or RotoWire RSS only; RSS updates are displayed as unreviewed news and do not automatically change model prices. SportsDataIO, Sleeper, RotoBaller, and licensed RotoWire integrations are listed as future provider candidates behind the same normalized feed shape.

The draft room is file-backed, not database-backed. Every sale, undo, reset, and import is serialized per active session and writes through the active session store before the server accepts the mutation. The real draft-night files are ignored by git and live at `data/live-draft/live-draft-current.json`, `data/live-draft/live-draft-backup.json`, and `data/live-draft/live-draft-log.jsonl`; preset practice rooms write under their own subdirectories so practice cannot accidentally mutate the real draft log. Live reset and import require both explicit UI confirmation and the current command count, which prevents stale tabs from overwriting a room that has moved on. On restart, the store loads the current snapshot first, then backup, then the append-only log if both snapshots are damaged. Use `MOCKD_LIVE_DRAFT_DIR=path/to/session npm run draft:ui` or `npm run draft:ui -- --session-dir=path/to/session` when you want a different real draft file set; its practice room will live under that directory. The engine still rebuilds state from raw `commands[]`, so restarting the server loads each current file and reconstructs the same room. The UI can export/import the active room's raw command log as JSON or CSV, shows exact raw commands in sales history, keeps the quick-sale command focused after actions, and surfaces Cam's current shortlist, owner needs/blockers, and live readiness checks.
