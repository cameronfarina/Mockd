...
The live UI also has an interactive mock panel for practice sessions. The mock uses a deterministic snake nomination order, the same player pricing and auction economics as batch mocks, historical owner personality profiles for non-Cam teams, and the selected Cam strategy for personal decision points. When the AI controls a non-Cam sale, it proposes the exact raw command that would be logged. When Cam can beat the top AI bid under the selected strategy, the panel pauses for `Cam Wins` or `Pass`. Mock actions intentionally mutate the active session log, so use a separate session directory for practice, such as `npm run draft:ui -- --session-dir=data/live-draft/practice-3rb`, and reserve the real draft-night directory for actual league sales.
