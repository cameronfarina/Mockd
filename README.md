...
Roster maximums are tuned to this league's historical draft shape: mocks cap owners at three QBs, six RBs, six WRs, two TEs, one kicker, and one defense so cheap late fillers do not crowd out RB/WR depth. Owner-specific history tightens that further for one-QB and one-TE owners, while owners who historically carried backups can still do it.
...
`teams` mines complete rosters from real mock batches for an owner and strategy. With `--strategy-mode=force`, the selected strategy is pushed into the auction engine before filtering results; with `--strategy-mode=filter`, the command only finds naturally occurring matches. The Cam true-three-RB strategy targets three premium RB slots, caps expensive fourth-RB depth, reserves room for paid WR starters, and reports the actual mock sale price plus the batch sale range for every player. Markdown reports include draft-path recommendations: max price bands, target clusters, pivot rules, and dead-zone warnings.
...
`draft:ui` starts the local draft room at `http://localhost:4317` by default. The left rail has three distinct draft actions: **Start real draft** for draft-night sale logging, **Start mock draft** for an interactive practice room where Cam controls Cam and AI owners control everyone else, and **Run mocks** for read-only AI-only batch simulations. The board is searchable, filterable by position, selected-owner needs, NFL team, and bye week, and sortable by the main draft-day economics columns. It uses `+` actions to stage a player for a selected team and sale price, shows expected price, live room-adjusted price, owner-specific personal value, value gap, recommended max bid, team, bye week, tier-drop tags, and position-market pressure. The strategy selector changes Cam's personal value, tags, shortlist, and recommended bids in real time without changing the league market anchor. `Our` remains Cam's strategy value, while `Max` now respects the selected strategy's path discipline, such as the true-three-RB $62/$54/$44 RB core ladder. The header also accepts compact sale commands such as `jakub kittle 28`. The side panel shows a selectable owner roster with QB/RB/RB/WR/WR/TE/FLEX/K/DST/bench slots, open needs, owner budgets, draft-path max bands, target clusters, pivot rules, recent sales, and warnings before impossible sales such as over-max bids or position-limit violations. Players outside the priced auction pool can still resolve from the 2026 ESPN projection dataset and are marked as projection fallback targets. The 2026 bye-week metadata lives in `config/nflTeams.ts`.

The draft room is file-backed, not database-backed. Every sale, undo, reset, and import writes through the active session store before the server accepts the mutation. The real draft-night files are ignored by git and live at `data/live-draft/live-draft-current.json`, `data/live-draft/live-draft-backup.json`, and `data/live-draft/live-draft-log.jsonl`; the interactive practice room writes to `data/live-draft/interactive-mock/` so practice cannot accidentally mutate the real draft log. Use `MOCKD_LIVE_DRAFT_DIR=path/to/session npm run draft:ui` or `npm run draft:ui -- --session-dir=path/to/session` when you want a different real draft file set; its practice room will live under that directory. The engine still rebuilds state from raw `commands[]`, so restarting the server loads each current file and reconstructs the same room. The UI can export/import the active room's raw command log as JSON or CSV, shows exact raw commands in sales history, keeps the quick-sale command focused after actions, and surfaces Cam's current shortlist, owner needs/blockers, and live readiness checks.
...
`Run mocks` starts a read-only AI-only batch without writing sale commands. The batch starts with the selected Cam strategy and cycles the other strategy variants for comparison. The button becomes a progress bar while simulations run; when it finishes, it changes to `See results` and opens `/mock-results`. That results page has a run selector such as `Run 1: 3rb` or `Run 2: balanced`, a strategy leaderboard, Cam score-range analytics, common Cam roster paths, a 3-by-5 grid with all 14 team drafts plus an AI rankings card, optimized Week 1 starters, player prices, Week 1 estimates, team Week 1 totals, and Weeks 1-4 score context. Each run also explains the projected ranking, Cam's outcome, the best build, and the most fragile build so the grid is more than a pile of rosters.
