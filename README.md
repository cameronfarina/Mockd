...
`draft:ui` starts the local live draft room at `http://localhost:4317` by default. The board is searchable, filterable by position, selected-owner needs, NFL team, and bye week, and sortable by the main draft-day economics columns. It uses `+` actions to stage a player for a selected team and sale price, shows expected price, live room-adjusted price, owner-specific personal value, value gap, recommended max bid, team, bye week, tier-drop tags, and position-market pressure. The strategy selector changes Cam's personal value, tags, shortlist, and recommended bids in real time without changing the league market anchor. The header also accepts compact sale commands such as `jakub kittle 28`. The side panel shows a selectable owner roster with QB/RB/RB/WR/WR/TE/FLEX/K/DST/bench slots, open needs, owner budgets, recent sales, and warnings before impossible sales such as over-max bids or position-limit violations. Players outside the priced auction pool can still resolve from the 2026 ESPN projection dataset and are marked as projection fallback targets. The 2026 bye-week metadata lives in `config/nflTeams.ts`.

The live draft room is file-backed, not database-backed. Every sale, undo, reset, and import writes through one session store before the server accepts the mutation. The default local files are ignored by git and live at `data/live-draft/live-draft-current.json`, `data/live-draft/live-draft-backup.json`, and `data/live-draft/live-draft-log.jsonl`; use `MOCKD_LIVE_DRAFT_DIR=path/to/session npm run draft:ui` or `npm run draft:ui -- --session-dir=path/to/session` when you want a different draft room file set. The engine still rebuilds state from raw `commands[]`, so restarting the server loads the current file and reconstructs the same room. The UI can export/import the raw command log as JSON or CSV, shows exact raw commands in sales history, keeps the quick-sale command focused after actions, and surfaces Cam's current shortlist, owner needs/blockers, and live readiness checks.

The live UI also has an interactive mock panel for practice sessions. The mock uses a deterministic snake nomination order, the same player pricing and auction economics as batch mocks, historical owner personality profiles for non-Cam teams, and the selected Cam strategy for personal decision points. When the AI controls a non-Cam sale, it proposes the exact raw command that would be logged. When Cam can beat the top AI bid under the selected strategy, the panel pauses with a concrete `Bid $X` action or `Pass`, and shows which AI owner/bid Cam is beating. Mock actions intentionally mutate the active session log, so use a separate session directory for practice, such as `npm run draft:ui -- --session-dir=data/live-draft/practice-3rb`, and reserve the real draft-night directory for actual league sales.
...
3. Add separate named live/mock sessions in the UI so practice cannot accidentally mutate the real draft log.
4. Add a web-app upload flow once the league-specific engine is trusted.
5. Add auth, durable hosted storage, provider adapters, and per-league calibration jobs before opening this to other leagues.
